...
void loop() {
  ...
  if(timeChange >=sampleRate)
  {
    ...
    */
    ...
    sensorVal4= constrain(sensorVal4,1200,2000);
...
also at this point, i assum that we've managed to figure out the sensors so we can appropriately set w_1-4
...
T_n α (w_n)^2
M=L X T_n where L is distance from the center of mass (theoretically the center but not really)
...
this is done by changing condition 3 in 'hover'
